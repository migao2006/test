...
assert.equal(health.version, "15.5");
...
const appResponse = await worker.fetch(new Request("https://example.test/app.js?v=15.5"), {}, {});
...
const smartResponse = await worker.fetch(new Request("https://example.test/smart.js?v=15.5"), {}, {});
const smartSource = await smartResponse.text();
assert.match(smartSource, /SMART SCREENER · \$\{VERSION\}/);
assert.match(smartSource, /listed: \{ revenue: 25/);
assert.match(smartSource, /otc: \{ revenue: 30/);
assert.match(smartSource, /etf: \{ liquidity: 35/);
assert.match(smartSource, /缺資料不會得到預設分/);
assert.match(smartSource, /await wait\(1500\)/);
assert.match(smartSource, /globalThis\.twssGroupRanking = groupRanking/);
